...
def java_komp(master):
    ...
    try:
        ...
        java_komp_list = [line.partition(".war")[0] for line in output[1:-1] if "jmx.war" not in line]
...
def open_new_component_popup(parent, style_font):
    ...
    def on_deploy_new():
        # Startet den Workflow mit is_new_component=True
        run_deployment_workflow(
            client, conn, cursor, baen,
            komponent_entry.get(),
            version_entry.get(),
            cluster_entry.get(),
...
update_btn = ttk.Button(btn_row, text="Aktualisieren", 
                        command=lambda: run_deployment_workflow(client, conn, cursor, baen, komponenten.get(), input_box.get(), cluster.get()))
update_btn.pack(side=tk.LEFT, padx=8)

new_btn = ttk.Button(btn_row, text="Neue Komponente", 
